...
def main(n: int) -> int:
    ...
    if n <= _max_n_for_fib_memo:
        ...
        print(f"Skipping evaluation of fib_memo b/c n >= {_max_n_for_fib_memo} and it'll blow the stack recurring given that input size")

    if n <= _max_n_for_fib_naive:
        algs.append(calc_fib_naive)
    else:
        print(f"Skipping evaluation of fib_naive b/c n >= {_max_n_for_fib_naive} and it's too slow for that input size")

    print(f"Note: not making an effort to warm the thread so timing values may vary widely")
